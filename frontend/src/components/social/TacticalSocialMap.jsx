...
export default function TacticalSocialMap({ events = [], selectedEvent, onSelectEvent }) {
  ...
  useEffect(() => {
    ...
    // Track mouse coordinates for HUD crosshair readout
    map.on('mousemove', (e) => {
      setCoords({
  ...
  useEffect(() => {
    ...
    filteredEvents.forEach(ev => {
      ...
      const popupHtml = `
        <div class="p-2.5 font-mono text-xs max-w-xs text-zinc-100">
          <div class="flex items-center justify-between border-b border-[#2d3b55] pb-1 mb-1.5">
            ...
            <span class="px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-widest" style="background-color: ${color}20; color: ${color}; border: 1px solid ${color};">
              ${ev.category}
            </span>
          </div>
      ...
      marker.on('click', () => onSelectEvent && onSelectEvent(ev));
