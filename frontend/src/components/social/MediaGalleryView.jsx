    ...
    <div className="h-full bg-[#07090e] border border-[#1c2538] flex flex-col tactical-box overflow-hidden font-mono min-h-[460px]">
      ...
      <div className="border-b border-[#1c2538] p-3 bg-[#0a0e17] flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <span className="px-1.5 py-0.5 bg-zinc-800 text-[10px] text-cyan-300 border border-zinc-700 font-bold">
      ...
      {/* Media Grid */}
      <div className="flex-1 overflow-y-auto p-3 max-h-[600px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredItems.map((item, idx) => {
              ...
              return (
                <div
                  key={`${item.url}-${idx}`}
                  className="group bg-[#0d121c] border border-[#1c2538] hover:border-cyan-500 transition-all flex flex-col overflow-hidden text-xs cursor-pointer shadow-lg hover:shadow-sm"
                  onClick={() => onOpenMedia && onOpenMedia(item)}
                >
